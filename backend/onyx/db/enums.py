...
class SignalType(str, PyEnum):
    ...
    CITATION_BAD = "citation_bad"
    UNSAFE = "unsafe"
    RESOLVED = "resolved"
    IGNORED = "ignored"


class OutcomeStatus(str, PyEnum):
    SUCCESS = "success"
...
class FinalResolution(str, PyEnum):
    ANSWERED = "answered"
...
class HealingTargetType(str, PyEnum):
    GLOBAL = "global"
    AGENT = "agent"
    FEATURE = "feature"


class RecommendationTargetType(str, PyEnum):
    PROMPT = "prompt"
    AGENT = "agent"
...
class RecommendationType(str, PyEnum):
    ...
    RETRIEVAL_K_CHANGE = "retrieval_k_change"
    MODEL_FALLBACK_RULE = "model_fallback_rule"
...
class RuleScopeType(str, PyEnum):
    GLOBAL = "global"
    AGENT = "agent"


class AutoOptimizationRuleType(str, PyEnum):
