...
class PromptTemplate(Base):
    ...
    assignments: Mapped[list["PromptAssignment"]] = relationship(
        "PromptAssignment",
        back_populates="template",
        cascade="all, delete-orphan",
    )
...
class PromptVersion(Base):
    ...
    prompt_template_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("prompt_template.id", ondelete="CASCADE"), nullable=False
    )
    ...
    __table_args__ = (
        UniqueConstraint(
            "prompt_template_id",
            "version_number",
            name="uq_prompt_version_template_id_version",
        ),
    )


class PromptAssignment(Base):
    __tablename__ = "prompt_assignment"
    ...
    template: Mapped["PromptTemplate"] = relationship(
        "PromptTemplate", back_populates="assignments"
    )
    ...
    __table_args__ = (
        UniqueConstraint(
            "target_type",
            "target_id",
            name="uq_prompt_assignment_target_type_id",
        ),
    )

class AgentRunTrace(Base):
    __tablename__ = "agent_run_trace"
    ...
    user_id: Mapped[UUID | None] = mapped_column(PGUUID(as_uuid=True), ForeignKey("user.id", ondelete="SET NULL"), nullable=True)
    prompt_template_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("prompt_template.id", ondelete="SET NULL"), nullable=True)
    prompt_version_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("prompt_version.id", ondelete="SET NULL"), nullable=True)
    model_name: Mapped[str] = mapped_column(String, nullable=False)
    retrieval_strategy: Mapped[str] = mapped_column(String, nullable=False)
    tool_plan_json: Mapped[dict | None] = mapped_column(postgresql.JSONB(), nullable=True)
    tools_called_json: Mapped[dict | None] = mapped_column(postgresql.JSONB(), nullable=True)
    citations_json: Mapped[dict | None] = mapped_column(postgresql.JSONB(), nullable=True)
    latency_ms: Mapped[int] = mapped_column(Integer, nullable=False)
    token_usage_json: Mapped[dict | None] = mapped_column(postgresql.JSONB(), nullable=True)
    outcome_status: Mapped[str] = mapped_column(String, nullable=False)
    confidence_score: Mapped[float | None] = mapped_column(Float, nullable=True)
    failure_reason: Mapped[str | None] = mapped_column(Text, nullable=True)
    recovery_actions_json: Mapped[dict | None] = mapped_column(postgresql.JSONB(), nullable=True)
    final_resolution: Mapped[str] = mapped_column(String, nullable=False)
    created_at: Mapped[datetime.datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)

class UserFeedbackSignal(Base):
    __tablename__ = "user_feedback_signal"

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    run_trace_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("agent_run_trace.id", ondelete="SET NULL"), nullable=True)
    user_id: Mapped[UUID | None] = mapped_column(PGUUID(as_uuid=True), ForeignKey("user.id", ondelete="SET NULL"), nullable=True)
    agent_id: Mapped[int | None] = mapped_column(Integer, nullable=True)
    ...
    metadata_json: Mapped[dict | None] = mapped_column(postgresql.JSONB(), nullable=True)
    created_at: Mapped[datetime.datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)
...
class HealingPolicy(Base):
    ...
    enabled: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True)
    low_confidence_threshold: Mapped[float | None] = mapped_column(Float, nullable=True)
    max_retries: Mapped[int] = mapped_column(Integer, nullable=False, default=3)
    allow_model_fallback: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True)
    allow_retrieval_expansion: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True)
    allow_retrieval_narrowing: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True)
    allow_tool_replan: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True)
    allow_prompt_fallback: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True)
    allow_human_escalation: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True)
    blocked_actions_json: Mapped[dict | None] = mapped_column(postgresql.JSONB(), nullable=True)
    environment: Mapped[str | None] = mapped_column(String, nullable=True)
    created_at: Mapped[datetime.datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    updated_at: Mapped[datetime.datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now(), nullable=False)

class LearningRecommendation(Base):
    __tablename__ = "learning_recommendation"

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    target_type: Mapped[str] = mapped_column(String, nullable=False)
    target_id: Mapped[str] = mapped_column(String, nullable=False)
    recommendation_type: Mapped[str] = mapped_column(String, nullable=False)
    current_config_json: Mapped[dict] = mapped_column(postgresql.JSONB(), nullable=False)
    proposed_config_json: Mapped[dict] = mapped_column(postgresql.JSONB(), nullable=False)
    evidence_json: Mapped[dict] = mapped_column(postgresql.JSONB(), nullable=False)
    confidence_score: Mapped[float | None] = mapped_column(Float, nullable=True)
    impact_estimate_json: Mapped[dict | None] = mapped_column(postgresql.JSONB(), nullable=True)
    status: Mapped[str] = mapped_column(String, nullable=False)
    created_at: Mapped[datetime.datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    reviewed_by_user_id: Mapped[UUID | None] = mapped_column(PGUUID(as_uuid=True), ForeignKey("user.id", ondelete="SET NULL"), nullable=True)
    reviewed_at: Mapped[datetime.datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)

class AutoOptimizationRule(Base):
    __tablename__ = "auto_optimization_rule"
    ...
    guardrails_json: Mapped[dict] = mapped_column(postgresql.JSONB(), nullable=False)
    created_at: Mapped[datetime.datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    updated_at: Mapped[datetime.datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now(), nullable=False)
